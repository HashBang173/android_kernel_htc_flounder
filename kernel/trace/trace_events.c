		    ...
		    struct ftrace_event_file *file, struct dentry *parent)
{
	...
	if (!entry) {
		kfree(system->filter);
		system->filter = NULL;
		pr_warning("Could not create tracefs '%s/filter' entry\n", name);
	}
		 ...
		 const struct file_operations *format)
{
	...
	file->dir = tracefs_create_dir(call->name, d_events);
	if (!file->dir) {
		pr_warning("Could not create tracefs '%s' directory\n",
			   call->name);
		return -1;
	}
...
create_event_toplevel_files(struct dentry *parent, struct trace_array *tr)
{
	...
	if (!entry) {
		pr_warning("Could not create tracefs 'set_event' entry\n");
		return -ENOMEM;
	}

	d_events = tracefs_create_dir("events", parent);
	if (!d_events) {
		pr_warning("Could not create tracefs 'events' directory\n");
		return -ENOMEM;
	}
